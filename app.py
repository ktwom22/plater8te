...
def nearby_restaurants():
    ...
    try:
        if GOOGLE_PLACES_API_KEY:
            url=f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={lat},{lon}&radius={int(radius_meters)}&type=restaurant&key={GOOGLE_PLACES_API_KEY}"
            ...
            for r in Restaurant.query.all():
                if r.latitude is None or r.longitude is None:
                    continue
                dist=haversine(float(lat), float(lon), r.latitude, r.longitude)
                if dist<=radius_meters/1609.34:
                    restaurants.append({"name":r.name,"latitude":r.latitude,"longitude":r.longitude,"address":r.address or ''})
    except Exception as e:
        return jsonify({'restaurants':[],'error':'Places error','detail':str(e)}),500
...
def play():
    ...
    plates_data = [{"id":p.id,"name":p.name,"description":p.description or '',"rating":0,"image_url":p.image_url or url_for('static',filename='uploads/placeholder.png'),"restaurant":{"name":p.restaurant.name if p.restaurant else ''}} for p in plates]
...
def my_plates():
    ...
    # plates this user has not yet rated
...
def unrated_plates():
    ...
    user_id = session['user_id']
    unrated = Plate.query.join(
        UserPlate, (UserPlate.plate_id == Plate.id) & (UserPlate.user_id == user_id), isouter=True
    ).filter((UserPlate.description == None) | (UserPlate.description == '')).all()
